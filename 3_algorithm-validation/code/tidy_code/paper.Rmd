...
Voluntary sharing in research articles.
  ...
  pbreplicate(
    ...
    expr = {
      eval_boot(
        ...
        pred_label = pred_label_vars[c(1, 4)]  # 1 algorithm, 3 definition
      )
    }, 
    simplify = F, 
    cl = 8
  ...
  pbreplicate(
    ...
    expr = {
      eval_boot(
        ...
        pred_label = pred_label_vars[c(1, 3)]
      )
    }, 
    simplify = F, 
    cl = 8
...
df_any <- 
  pbreplicate(
    n = params$n, 
    expr = eval_boot(type = "any", is_old = F, filters = filtering_vars[0]), 
    simplify = F, 
    cl = 8
  ) %>% 
  bind_rows()
...
eval_tables <- list(
  ...
  Data = out_data$data_eval_voluntary,
...
p1 <- 
  ...
  mutate(vals = sprintf("%0.1f%% \n(%0.1f%%-%0.1f%%)", median, lo, hi)) %>% 
  ggplot(aes(x = Indicator, y = metric, fill = median, label = vals)) +
...
p2 <- 
  ...
  ggplot(aes(x = Indicator, y = metric, label = vals, fill = median)) +
  geom_tile(aes(width = 0.99, height = 0.99)) +
  geom_text(size = 3) +
