...
These are random samples taken from all fields with clusters with >250 articles. These were randomly identified using the random number generator of Excel by Kevin Boyack.

```{r}
# Import
field_reps <- read_xlsx(
  path = "../../data/external_data/field-representatives.xlsx", 
  sheet = 2
)
...
field_reps %<>% 
  select(-PM5, -np1519) %>% 
  select(Field, Clusters = 2, Journals = 3, Reviews = 4) %>% 
  mutate(Reviews = str_replace(Reviews, "^.*; (.*?)\\..*$", "\\1")) %>%
  group_by(Field) %>% 
...
write_csv(
  field_reps, 
  "../../output/table_output/indicator-eval_field-reps.csv"
)

# Print
field_reps %>% sable(full_width = T)
```
