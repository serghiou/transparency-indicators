...
# Import data
meta <- vroom("../../data/tidy_data/pmcoa-meta-data_all.csv")
pmcoa <- vroom("../../data/tidy_data/indicators_all.csv")

# Import validation data (out_data, out_code, out_coi, out_fund, out_register)
data_output <- "../../../3_algorithm-validation/output/data_output"
load(file.path(data_output, "paper_eval-lists.RData"))
```
...
A function to easily create counts and proportions.
...
A function to create a summary table for numeric features.

```{r}
tabulate_nums <- function(.data, 
                          indicator_var = indicator_vars[1], 
                          num_vars = num_vars_meta) {
  ...
  .data %>% 
    ...
    pivot_longer(
      ...
      names_to = "vars",
      values_to = "vals") %>% 
    mutate(vars = recode(vars, !!! dic)) %>% 
    group_by(vars) %>% 
    mutate(p_value = kruskal.test(vals ~ !!! indicator_var)$p.value) %>% 
    group_by(vars, !!! indicator_var, p_value) %>% 
    summarise_at(vars(vals), list(
      mean = ~ mean(., na.rm = T), 
      SD = ~ sd(., na.rm = T), 
    ...
    ungroup() %>% 
    gather(indicator, value, !!! indicator_var) %>% 
    mutate(indicator = recode(indicator, !!! dic)) %>% 
    select(indicator, value,  everything()) %>% 
    select(-p_value, p_value) %>% 
    mutate_at(vars(Q1:Q3), formatC, digits = 0, format = "f")
  
}
```


A function to prettify the table of numeric features created above.

```{r}
prettify_num_table <- function(.data, indicator_var = indicator_vars[1]) {
  
  .data %>% 
    mutate(vals = glue::glue("{Q2} ({Q1}-{Q3})")) %>% 
    mutate(value = if_else(value, "Present", "Absent")) %>% 
    select(1:3, vals, p_value) %>% 
...
indicator_text_vars_dic <- c(
  "data_text" = "Data sharing",
  "code_text" = "Code sharing",
  "coi_text" = "COI disclosure",
  "fund_text" = "Funding disclosure",
  "register_text" = "Protocol registration"
)
...
performance_error <- list(
  is_data_pred = out_data$data_eval_active,
  is_code_pred = out_code$code_eval_active,
  is_coi_pred  = out_coi$coi_eval_any,
  is_fund_pred = out_fund$fund_eval_any,
  is_register_pred = out_register$reg_eval_any
) %>% 
  bind_rows(.id = "indicator") %>% 
  filter(metric %in% c("NPV", "PPV")) %>% 
  mutate_at(vars(median:expected), magrittr::divide_by, 100) %>% 
  mutate(value = if_else(metric == "PPV", "Yes", "No")) %>% 
  gather(estimand, estimate, expected, lo, hi) %>% 
  # pivot_wider(names_from = metric, values_from = median:hi) %>% 
  mutate(indicator = recode(indicator, !!!dic))

# Research articles
performance_error_art <- list(
  is_data_pred = out_data$data_eval_active_art,
  is_code_pred = out_code$code_eval_active_art,
  is_coi_pred  = out_coi$coi_eval_any_art,
  is_fund_pred = out_fund$fund_eval_any_art,
  is_register_pred = out_register$reg_eval_any_art
) %>% 
  bind_rows(.id = "indicator") %>% 
  filter(metric %in% c("NPV", "PPV")) %>%
  mutate_at(vars(median:expected), magrittr::divide_by, 100) %>% 
...
pmc <- 
  ...
  mutate(pmcid_pmc = as.character(pmcid_pmc))
```
...
```{r}
meta %<>% 
  select(pmcid_pmc, pmid, doi, title, n_auth:is_supplement) %>% 
  mutate_at(vars(pmcid_pmc, pmid), as.character) %>% 
  mutate(n_fig = n_fig_body + n_fig_floats) %>% 
  mutate(n_table = n_table_body + n_table_floats) %>% 
  select(-c(n_fig_body:n_table_floats)) %>% 
  select(-n_fig, -n_table, n_fig, n_table)

meta <- 
  pmc %>% 
  select(pmcid_pmc, year, ends_with("_pred"), !!!num_vars, !!!factor_vars) %>%
  inner_join(meta, by = "pmcid_pmc") %>% 
  select(pmcid_pmc, pmid, doi, everything())
```
...
corrected <- 
  ...
  mutate(p_corrected = glue::glue("{expected} ({hi}-{lo})")) %>% 
...
corrected <- 
  ...
  count(value) %>% 
  mutate(N = sum(n)) %>% 
  left_join(performance_error_art, by = c("key" = "indicator", "value")) %>% 
  mutate(true = n * estimate, false = n * (1 - estimate)) %>% 
  gather(true_false, counts, true:false) %>% 
  mutate(is_present = case_when(
    metric == "PPV" & true_false == "true" ~ "present",
...
ggsave("../../output/figure_output/indicator-eval_indicator-counts-by-pmc.jpg")
```
...
Number of articles published ≥ 2000.

```{r}
sprintf("No. published ≥ 2000 = %s", nrow(filter(meta, year >= 2000)))
```


All statistics - all articles, 2000 onward.

```{r}
# Tabulate (this is an expensive operation)
num_table <- 
  indicator_vars %>% 
  purrr::map_dfr(~ tabulate_nums(meta, .x, c(num_vars, num_vars_meta)))

# Save
write.csv(num_table, "../../output/table_output/indicator-eval_num-table.csv")
...
Median and IQR - all articles, 2000 onward.

```{r}
# Save
prettify_num_table(num_table) %>%
  write.csv("../../output/table_output/indicator-eval_num-table-pretty.csv")
...
ggsave(
  filename = "../../output/figure_output/indicator-eval_indicator-by-year.jpg", 
  width = 11,
  height = 7
)
```
...
ggsave(
  "../../output/figure_output/indicator-eval_indicator-by-year_art.jpg", 
  width = 11,
  height = 7
)
```
...
ggsave(
  "../../output/figure_output/indicator-eval_indicator-by-country.jpg", 
  width = 12, 
  height = 12
)
```
...
ggsave(
  "../../output/figure_output/indicator-eval_field-by-indicator_unscaled.jpg",
  width = 8, 
  height = 6
)
```
...
ggsave(
  "../../output/figure_output/indicator-eval_field-by-indicator_scaled.jpg",
  width = 8, 
  height = 6
)
```
...
ggsave(
  "../../output/figure_output/indicator-eval_field-by-ind_art_unscaled.jpg", 
  width = 8, 
  height = 6
)
```
...
ggsave(
  "../../output/figure_output/indicator-eval_field-by-ind_art_scaled.jpg", 
  width = 8, 
  height = 6
)
```
...
Identify the titles of 15 random articles from each field.
...
field_titles <- 
  meta %>% 
  select(field, title, journal, n_cite) %>% 
  drop_na(field, n_cite) %>% 
  group_by(field) %>% 
  sample_n(15) %>% 
  arrange(field, -n_cite) %>% 
  rename(
    Field = field, Title = title, Journal = journal, Citations = n_cite
  ) 

# Save
field_titles %>% 
  write.csv("../../data/interim_data/indicator-eval_field-titles.csv")
```


Select the 3 most representative titles from each field.

```{r}
# Import selections
field_titles <- read_excel(
  "../../data/interim_data/indicator-eval_field-titles_complete.xlsx"
)
...
# Save
field_titles %>% 
  write_csv("../../output/table_output/indicator-eval_field-titles.csv")
...
The extracted text for the most cited article with a positive indicator. Using replace = T because certain categories have less than 10 articles with specific indicators. The data were arranged in citation descending order and the best example in terms of (a) a correct prediction, (b) a short text and (c) as high citation as possible was chosen.
...
field_text <- 
  pmc %>% 
  select(field, ends_with("_text"), pmcid_pmc, n_cite) %>% 
  pivot_longer(ends_with("_text"), "indicator") %>% 
  drop_na(field, value) %>% 
  group_by(field, indicator) %>% 
  sample_n(10,  replace = T) %>% 
  ungroup() %>% 
  arrange(field, indicator, -n_cite) %>% 
  select(-n_cite, -pmcid_pmc, pmcid_pmc, n_cite) %>% 
  mutate(indicator = recode(indicator, !!! indicator_text_vars_dic))

# Save
field_text %>% 
  write.csv("../../data/interim_data/indicator-eval_field-text.csv")
```

```{r}
# Import selections
field_text <- 
  read_excel("../../data/interim_data/indicator-eval_field-text_complete.xlsx")
...
# Save
field_text %>% 
  write_csv("../../output/table_output/indicator-eval_field-text.csv")

# Print
field_text %>% sable(full_width = T)
```


## Galaxies

Number of articles between 2015-2019.
...
Number of articles per indicator.
...
ggsave(
  "../../output/figure_output/indicator-eval_indicator-by-journal.jpg", 
  width = 10, 
  height = 6
)
```
...
ggsave(
  "../../output/figure_output/indicator-eval_indicator-by-journal_art.jpg",
  width = 10, 
  height = 6
)
```
...
ggsave(
  "../../output/figure_output/indicator-eval_indicator-by-publisher.jpg", 
  width = 10, 
  height = 6
)
```
...
ggsave(
  "../../output/figure_output/indicator-eval_indicator-by-publisher_art.jpg",
  width = 10, 
  height = 6
)
```
